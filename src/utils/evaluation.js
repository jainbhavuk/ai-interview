...
export function evaluateAnswer(answer, question) {
  ...
  const hasContext = /(when|while|during|at|project|team|customer)/i.test(
    cleanAnswer,
  );
  const hasTradeOff =
    /(trade[\s-]?off|because|decided|alternative|constraint)/i.test(
      cleanAnswer,
    );
  const hasAction = /(built|improved|implemented|designed|fixed|led)/i.test(
    cleanAnswer,
  );
...
export function buildInterviewReport({
  ...
  const scores = answered?.map((entry) => entry?.evaluation?.score || 0) || [];
  ...
  answered?.forEach((entry) => {
    ...
    list.push(entry?.evaluation?.score || 0);
    competencyMap.set(key, list);
  });

  const competencyScores = [...competencyMap.entries()]?.map(
    ([competency, values]) => ({
      competency,
  ...
  answered?.forEach((entry) => {
    findMentionedSkills(`${entry?.prompt || ''} ${entry?.answer || ''}`)?.forEach((skill) =>
      coveredSkills?.add(skill),
    );
  });

  const matchedSkills = jdProfile?.requiredSkills?.filter((skill) =>
    coveredSkills?.has(skill),
  ) || [];
  const missingRequiredSkills = jdProfile?.requiredSkills?.filter(
  ...
  const strengths = answered
    ...
    ?.map(
      (entry) =>
        `Strong ${entry?.competency || "general"} response: "${entry?.prompt || "N/A"}"`,
    ) || [];

  const lowSignalEntries = answered
    ?.filter((entry) => (entry?.evaluation?.score || 0) <= 3)
    ?.slice(0, 2)
    ?.map((entry) => `Improve depth for: "${entry?.prompt || "N/A"}"`) || [];

  const improvements = [
    ...(lowSignalEntries || []),
    ...(missingRequiredSkills || [])
      ?.slice(0, 3)
      ?.map((skill) => `Practice role-specific examples for "${skill}".`) || [],
  ];
  ...
  return {
    ...
    durationMinutes,
    overallScore,
    totalAnswers: answered?.length || 0,
    competencyScores,
    strengths: strengths.length
      ? strengths
      : ["Your answers were consistent; focus on adding stronger metrics."],

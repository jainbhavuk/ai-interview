...
function findTemplateById(templateId) {
  return (
    INTERVIEW_TEMPLATES.find((template) => template.id === templateId) ||
...
function createPersonalizedQuestions(profile, jdProfile) {
  const questions = [];
  const matchedSkills = getSharedSkills(
    profile?.skills || [],
  ...
  if (profile?.projects?.[0]) {
    questions.push(
      createQuestion(
        `Walk me through this resume claim: "${profile?.projects?.[0] || 'project'}". What was the problem, solution, and measurable impact?`,
        "communication",
        "resume-project",
      ),
    );
...
export function buildInterviewPlan(config) {
  ...
  const personalizedQuestions = createPersonalizedQuestions(
    resumeProfile,
    jdProfile,
  );
  const maxMainQuestions = clamp(
    Math.round((config.durationMinutes || 20) / 3),
    4,
    9,
  );

  const introQuestion = createQuestion(
    `Hi ${config?.candidateName || 'Candidate'}. Give me a 60-second introduction tailored to this role.`,
    "communication",
    "intro",
  );
...
function includesDepthSignal(answer) {
  return /(because|trade[\s-]?off|challenge|decision|first|second|finally|impact)/i.test(
    answer,
  );
}
...
export function generateFollowUpQuestion({ question, answer, jdProfile }) {
  ...
  if (wordCount < 30) {
    return {
      id: createQuestionId(),
      prompt:
        "Could you go one level deeper and walk through your exact approach step by step?",
      competency: question?.competency || "technical",
  ...
  if (!includesMetric(cleanAnswer)) {
    return {
      id: createQuestionId(),
      prompt:
        "What measurable outcome did this produce (latency, revenue, quality, speed, or user impact)?",
  ...
  if (
    !includesDepthSignal(cleanAnswer) &&
    question?.competency === "technical"
  ) {
    return {
      id: createQuestionId(),
      prompt:
  ...
  if (
    question?.skillTag &&
    jdProfile?.requiredSkills?.includes(question.skillTag)
  ) {
    return {
      id: createQuestionId(),
      prompt: `If the team scales traffic by 3x, what would you change first in your ${question?.skillTag || 'skill'} approach?`,
